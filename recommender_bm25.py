...
def bm25_vectorizer(corpus, k1=1.5, b=0.75):
    #tạo ma trận bm25 
    vectorizer = CountVectorizer(stop_words=VIETNAMESE_STOP_WORDS)
    X = vectorizer.fit_transform(corpus)
    # Chuyển sang dạng mảng để dễ thao tác
    term_freq = X.toarray()
    # Tính document frequency 
    df = np.sum(term_freq > 0, axis=0)
    # Tính IDF theo công thức BM25
    N = X.shape[0]
    idf = np.log((N - df + 0.5) / (df + 0.5) + 1)
    # Tính độ dài văn bản
    doc_lengths = np.sum(term_freq, axis=1)
    avg_doc_length = np.mean(doc_lengths)
    # Tạo ma trận kết quả
    bm25_scores = np.zeros_like(term_freq, dtype=np.float64)
    # Áp dụng công thức BM25 cho từng tài liệu
    for i in range(term_freq.shape[0]):
        tf = term_freq[i]
        numerator = tf * (k1 + 1)
        denominator = tf + k1 * (1 - b + b * doc_lengths[i] / avg_doc_length)
        bm25_scores[i] = numerator / denominator * idf
    return bm25_scores
def build_recommender(df, force_rebuild=False):
    cache_file = "bm25_matrix.pkl"
    if os.path.exists(cache_file) and not force_rebuild:
        with open(cache_file, 'rb') as f:
            return pickle.load(f)
    print("Đang xây dựng ma trận BM25...")
    texts = df['soup'].fillna('').astype(str).apply(normalize_text).tolist()
    bm25_matrix = bm25_vectorizer(texts)
    sim_matrix = cosine_similarity(bm25_matrix)
