...
def normalize_vietnamese_text(text):
    ...
    # Loại bỏ mã HTML
    text = re.sub(r'<.*?>', ' ', text)
    # Thay thế URL bằng token đặc biệt
    text = re.sub(r'https?://\S+|www\.\S+', ' url ', text)
    # Tách các từ bị dính với số
    text = re.sub(r'(\d+)([a-zA-Z])', r'\1 \2', text)
    text = re.sub(r'([a-zA-Z])(\d+)', r'\1 \2', text)
    # Thay thế email bằng token đặc biệt
    text = re.sub(r'\S+@\S+', ' email ', text)
    # Loại bỏ các ký tự đặc biệt và giữ lại tiếng Việt
...
def preprocess_text_with_pyvi(text):
    #tiền xử lý văn bản bằng pyvi 
    text = normalize_vietnamese_text(text)
    # Sử dụng PyVi để tách từ
    tokenized_text = ViTokenizer.tokenize(text)
    return tokenized_text
def filter_stopwords(tokens):
    #lọc bỏ stop word ra khỏi token 
    return [token for token in tokens if token.lower() not in VIETNAMESE_STOP_WORDS]
def build_recommender(df, force_rebuild=False):
    cache_file = "model_cache/improved_doc2vec_matrix.pkl"
    ...
    #tiền xử lý dữ liệu 
    processed_texts = df['soup'].fillna('').astype(str).apply(preprocess_text_with_pyvi).tolist()
    # Tạo dữ liệu gắn thẻ cho Doc2Vec
    tagged_data = []
    ...
    model = Doc2Vec(
        vector_size=200,        # số chiều vector
        window=8,              #  cửa sổ ngữ cảnh
        min_count=2,           # bỏ qua từ xuất hiện ít hơn 2 lần
        dm=1,                  # sử dụng mô hình Distributed Memory
        alpha=0.025,           # alpha ban đầu
        min_alpha=0.001,       # alpha tối thiểu
        workers=4,             # số luồng xử lý
        epochs=50,             # tăng số epochs
        hs=0,                  # sử dụng negative sampling
        negative=5             # số lượng negative samples
    )
    # Xây dựng từ vựng
    model.build_vocab(tagged_data)
    # Huấn luyện mô hình
    model.train(tagged_data, total_examples=model.corpus_count, epochs=model.epochs)
    # Tạo vectors cho tất cả tài liệu
    vectors = []
    for doc in tagged_data:
        # Sử dụng infer_vector để lấy vector biểu diễn tài liệu
    ...
    return sim_matrix
def recommend(df, sim_matrix, title, top_k=5):
    ...
    results = df.iloc[result_indices].to_dict(orient='records')
    for i, idx in enumerate(result_indices):
        results[i]['similarity_score'] = float(scores[i][1])
