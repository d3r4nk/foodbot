...
# Singleton pattern cho VnCoreNLP để tránh khởi tạo nhiều lần
class VnCoreNLPSingleton:
    _instance = None
    @classmethod
    def get_instance(cls, annotator_path='vncorenlp/VnCoreNLP-1.1.1.jar'):
        if cls._instance is None:
            print("Khởi tạo VnCoreNLP...")
            try:
                cls._instance = VnCoreNLP(annotator_path, annotators="wseg", max_heap_size='-Xmx2g')
                print("Khởi tạo VnCoreNLP thành công!")
            except Exception as e:
                print(f"Lỗi khởi tạo VnCoreNLP: {e}")
                # Fallback: trả về một hàm tokenize đơn giản nếu VnCoreNLP không khởi tạo được
                cls._instance = None
        return cls._instance
# Hàm chuẩn hóa và tách từ tiếng Việt bằng VnCoreNLP
def tokenize_text(text, annotator=None):
    ...
    if annotator is not None:
        try:
            ...
            # Nối các từ đã tách với dấu cách
            tokenized = ' '.join(' '.join(tokens) for tokens in sentences)
            return tokenized
        except Exception as e:
            print(f"Lỗi khi tokenize: {e}")
    # Fallback: trả về text đã được chuẩn hóa nếu không dùng được VnCoreNLP
    return text
...
def build_recommender(df, force_rebuild=False):
    #xây dựng ma trận tương đồng dựa trên tf-idf với vncorenlp 
    cache_file = "vncorenlp_matrix.pkl"
    # Kiểm tra cache
    if os.path.exists(cache_file) and not force_rebuild:
        with open(cache_file, 'rb') as f:
            return pickle.load(f)
    print("Đang xây dựng ma trận với VnCoreNLP...")
    # Khởi tạo VnCoreNLP
    annotator = VnCoreNLPSingleton.get_instance() 
    # Tiền xử lý dữ liệu với VnCoreNLP
    print("Đang tiền xử lý dữ liệu...")
    processed_texts = []
    for text in df['soup'].fillna('').astype(str).tolist():
        processed = tokenize_text(text, annotator)
        processed_texts.append(processed)
    # tạo TF-IDF với các tham số như bên recommender_tf_idf 
    ...
    print("Đang tính ma trận tương đồng...")
    sim_matrix = cosine_similarity(tfidf_matrix)
    # Lưu vào cache
    print(f"Đang lưu ma trận vào {cache_file}...")
