...
import os
import pickle
def normalize_text(text): #tiền xử lý văn bản tiếng việt 
...
def build_recommender(df, force_rebuild=False):
    cache_file = "tfidf_improved_matrix.pkl"
    # Kiểm tra cache 
    if os.path.exists(cache_file) and not force_rebuild:
        with open(cache_file, 'rb') as f:
            return pickle.load(f)
    # Tiền xử lý dữ liệu
    df['processed_text'] = df['soup'].fillna('').astype(str).apply(normalize_text)
    texts = df['processed_text'].tolist()
    # Tạo TF-IDF với tham số cải tiến
    ...
    tfidf = TfidfVectorizer(
        ...
        max_df=0.95,             # Bỏ qua các từ xuất hiện trong >95% tài liệu
        ngram_range=(1, 2),      # Dùng cả unigram và bigram
        stop_words=list(VIETNAMESE_STOP_WORDS) + ['the', 'and', 'to', 'of', 'a', 'in', 'is', 'for'],
        sublinear_tf=True        # Áp dụng logarithm scaling cho term frequencies
    )
    # Tạo ma trận TF-IDF
